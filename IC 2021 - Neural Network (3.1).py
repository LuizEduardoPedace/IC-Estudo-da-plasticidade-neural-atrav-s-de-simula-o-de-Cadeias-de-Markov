                ...
                if gj not in rep:  # Optimize the plot
                    ...
                    if gj < 0:
                        nx.draw_networkx_edges(G, nx.circular_layout(G), edgelist=weighted_edges, arrowsize=15,
                                               width=abs(gj) / 5, connectionstyle='Arc3, rad=0.15', ax=axe, edge_color='r')
                    else:
                        nx.draw_networkx_edges(G, nx.circular_layout(G), edgelist=weighted_edges, arrowsize=15,
...
w0 = float(input('Insert W: '))  # Here I choose the W
w = np.ones((n, n)) * w0
np.fill_diagonal(w, 0)
for nwi in range(n):
    for nwj in range(n):
        w[nwi][nwj] *= sample([-4, -4, 1, 1, 1, 1, 1, 1, 1, 1], 1)[0]  # Inhibitory neuron

# Matrix for type of neuron
ntype = np.array(w) / abs(w0)
        ...
        if x[j][time] == 1:
            for i in range(n):
                if L[j] < L[i] and x[i][time] == 0:
                    ...
                    if w[i][j] / ntype[i][j] < 0: w[i][j] = 0  # Roof for inhibitory neurons
                    if w[i][j] > 20: w[i][j] = 20  # Roof for excitatory neurons
                    w[j][i] += gamma(L[i] - time)
                    if w[j][i] / ntype[j][i] < 0: w[j][i] = 0  # Floor for excitatory neurons
                    if w[j][i] < -20: w[j][i] = -20  # Floor for inhibitory neurons
...
while gN != -1:
    if gN == -2:
        ...
        np.fill_diagonal(w, limit+1)  # Exclude principal diagonal  of plot
        sns.histplot(data=[val for boxi in w for val in boxi if val != limit+1], binwidth=1, stat='density', kde=True, color='b')
    else:
        if gN == 0:
            for g0 in range(n):
